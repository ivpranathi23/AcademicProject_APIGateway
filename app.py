...
def jsonResponse(statusCode, message):
	...
	return flask.jsonify(ContentLanguage="en-US", ContentType = "application/json", status_code=statusCode, Message=message), statusCode
...
@app.route('/v1users/createUser', methods=['POST'])
def createUser():
...
@app.route('/v1users/getUsers', methods=['GET'])
def getUsers():
...
@app.route('/v1users/authenticateUser', methods=['POST'])
def authenticateUser():
	...
	if username is None or password is None:
		return jsonResponse(401, "Authentication failed!!Missing Username or Password fields")
	...
	if userPassword is None:
		return jsonResponse(401, "Authentication failed!! User not found")
	else:
		...
		if result == True:
			...
			#return flask.jsonify(ContentLanguage="en-US", ContentType = "application/json", status_code=401, Message=result, Error="Authentication Failed")
			return flask.jsonify(ContentLanguage="en-US", ContentType = "application/json", status_code=401, Message=result, Error="Authentication Failed"), 401
...
@app.route('/v1users/addFollower', methods=['POST'])
def addFollower():
	...
	checkduplicate = query_db('Select username from followers where username = ? AND followerUsername = ?', [username, usernameToFollow], one=True)

	if checkduplicate:
		return jsonResponse(409, "You are already following this user")
...
@app.route('/v1users/removeFollower', methods=['POST'])
def removeFollower():
	...
	isRemovedAlready = query_db('Select username from FOLLOWERS where username = ? AND followerUsername=?', [username, usernameToRemove], one=True)
	if isRemovedAlready is None:
		return jsonResponse(400, "User already removed")
