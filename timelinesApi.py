...
@app.route('/v1timelines/postTweet', methods=['POST'])
def postTweet():
...
@app.route('/v1timelines/userTimeline', methods=['GET'])
def getUserTimeline():

	#Retrieves username and post text from the GET request
	username = request.args.get('searchUsername')
	
	#Checks whether username and post are NIL or not. If they are NIL, return error message
	if username is None:
		return jsonResponse(400,"Missing searchUsername")
...
@app.route('/v1timelines/publicTimeline', methods=['GET'])
...
#def getHomeTimeline is used to retrieve recent 25 posts from all the users, that given user followers. This is a GET Method
@app.route('/v1timelines/homeTimeline', methods=['GET'])
def getHomeTimeline():
